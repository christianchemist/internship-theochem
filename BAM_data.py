...
array_list = [data["bam"]["full structure"], data["bam"]["short dimer"], data["bam"]["long dimer"], data["bam"]["monomer"], data["bam"]["2x"], data["bam"]["4x"], data["bases monomers"]["adenine"], data["bases monomers"]["cytosine"], data["bases monomers"]["guanine"], data["bases monomers"]["thymine"], data["bam"]["2x no bases"], data["bam"]["4x no bases"]]
array_names = ["full structure", "short dimer", "long dimer", "monomer", "2x Chromophore", "4x Chromophore", "adenine monomer", "cytosine monomer", "guanine monomer", "thymine monomer", "2x Chromophores no bases", "4x Chromophores no bases"]
dataset_names = ["DFT", "DFTB"]
fourx = data["bam"]["4x"]
fourx_no = data["bam"]["4x no bases"]
comp = np.vstack((fourx[0], fourx_no[0]))
twox = data["bam"]["2x"]
twox_no = data["bam"]["2x no bases"]
comp2 = np.vstack((twox[0], twox_no[0]))
...
if __name__ == "__main__":
    ...
    if mode == "dft":
        ...
        plot_energy_comparison(comp2, list_setnames=["2x Chromophores", "2x Chromophores no bases"], mode="connected", title="2x Chromophore energy comparison plot")
        for array, name in zip(array_list, array_names):
